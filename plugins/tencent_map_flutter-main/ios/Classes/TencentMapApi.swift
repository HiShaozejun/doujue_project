...
class _TencentMapApi: NSObject {
  ...
  func updateMarker(markerId: String, options: MarkerUpdateOptions) {
    ...
    func addPolylines(lines: [CLLocationCoordinate2D]) {
       
        self.clearPolylines()
        
        let polyline = QPolyline(coordinates: coordinates, count: UInt(coordinates.count))
        polyline.color = UIColor(red: 0.98, green: 0.58, blue: 0.22, alpha: 1.0) // #f89437
        polyline.width = 12.0
        polyline.borderColor = .clear
        
        self.mapView.add(polyline)
        self.polylines.append(polyline)
        
        // 调整地图视野
        self.zoomToPolyline(coordinates: coordinates)
     }
