  ...
  func onMethodCall(call: FlutterMethodCall, result: FlutterResult) {
    if(call.method == "updateMapConfig") {
      ...
      result(nil)
        ...
        let arguments = call.arguments as! Dictionary<String, AnyObject>
        let rawPoints = arguments["polylines"] as? [[Double]]
        guard let rawPoints = rawPoints else {
            result(FlutterError(code: "INVALID_ARGUMENT", message: "Invalid polyline points", details: nil))
            return
        }
        
        let coordinates: [CLLocationCoordinate2D] = rawPoints.map { CLLocationCoordinate2D(latitude: $0[0], longitude: $0[1]) }
        
        DispatchQueue.main.async {
            api.addPolylines(lines: coordinates)
