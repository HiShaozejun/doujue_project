...
import 'package:djqs/base/widget/drag_float/AnchoringPosition.dart';
...
class _DraggableWidgetState extends State<DraggableWidget>
    ...
    if (WidgetsBinding.instance != null) {
      WidgetsBinding.instance!.addPostFrameCallback((timeStamp) async {
        ...
        setState(() {
          ...
          if (widget.initialPosition == AnchoringPosition.left) {
            top = boundary - widgetHeight + widget.statusBarHeight;
            left = MediaQuery.of(context).size.width - widgetWidth;
          } else if (widget.initialPosition == AnchoringPosition.right) {
            top = boundary - widgetHeight + widget.statusBarHeight;
            left = MediaQuery.of(context).size.width - widgetWidth;
          } else if (widget.initialPosition == AnchoringPosition.bottomRight) {
            top = boundary - widgetHeight + widget.statusBarHeight;
            left = MediaQuery.of(context).size.width - widgetWidth;
  ...
  AnchoringPosition determineDocker(double x, double y) {
    ...
    if (x <= totalWidth / 2) {
      return AnchoringPosition.left;
    } else if (x > totalWidth / 2) {
      return AnchoringPosition.right;
    ...
    switch (docker) {
      case AnchoringPosition.left:
        setState(() {
          ...
          // }
          currentlyDocked = AnchoringPosition.left;
        });
        break;
      case AnchoringPosition.right:
        ...
        setState(() {
          ...
          //       (widget.topMargin * (animation.value)));
          // }
          currentlyDocked = AnchoringPosition.right;
        });
        break;
