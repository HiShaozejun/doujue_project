...
class BaseDioUtil {
  ...
  bool isRefreshToken = false; //not use

  BaseDioUtil._internal() {
    _dio = createDio(BaseNetConst().commonUrl);
  }

  factory BaseDioUtil() => _instance;

  static late final BaseDioUtil _instance = BaseDioUtil._internal();
  ...
  request<T>(
    ...
    if (response?.statusCode == 200) {
      ...
      try {
        ...
        //
        if (baseEntity.code == 2000 ||
            baseEntity?.ret == BaseNetConst.REQUEST_OK) {
          if (returnBaseEntity)
            return isBaseResult ? baseEntity as T : baseEntity.data as T;
  ...
  _dealWithFunError(FuncEntity? entity, showFunMsg) async {
